...
const HomeChat: React.FC<HomeChatProps> = ({ t, lang }) => {
  ...
  // Forced Production Auto-Scroll with MutationObserver
  useEffect(() => {
    const scrollHandler = () => {
  ...
  return (
    <div className={`flex-1 flex flex-col h-[calc(100vh-64px)] ${isDarkMode ? 'bg-[#121212]' : 'bg-white'} overflow-hidden relative`}>
      <div ref={chatContainerRef} className="chat-container flex-1 overflow-y-auto flex flex-col p-4 md:p-8 space-y-10 pb-[160px]">
        {!activeSession || activeSession.messages.length === 0 ? (
          ...
          <div className="max-w-4xl mx-auto w-full space-y-12">
            {activeSession.messages.map((msg) => {
              ...
              return (
                <div key={msg.id} className={`flex items-start gap-5 ${msg.role === 'user' ? 'flex-row-reverse' : ''} animate-in fade-in slide-in-from-bottom-4`}>
                  ...
                  <div className={`relative max-w-[85%] px-6 py-5 rounded-[28px] shadow-sm ${msg.role === 'user' ? 'bg-slate-100 text-black border border-slate-200' : 'bg-slate-800 text-slate-100 border border-slate-700'}`}>
                    {isMedia ? (
                      <video src={urls[0] || msg.content} className="w-full rounded-2xl" controls autoPlay loop muted playsInline />
                    ) : (
        ...
        {isTyping && <div className="max-w-4xl mx-auto w-full flex gap-2 animate-pulse"><div className="w-2 h-2 bg-slate-400 rounded-full animate-bounce" /></div>}
      </div>

      <div className={`fixed bottom-0 left-0 right-0 p-4 md:p-8 z-30 ${isDarkMode ? 'bg-gradient-to-t from-[#121212]' : 'bg-gradient-to-t from-white'}`}>
        <div className="max-w-4xl mx-auto relative">
          <div className="flex items-end gap-3 p-3 rounded-[35px] border-2 bg-slate-100 border-slate-200 shadow-2xl">
            <button onClick={() => fileInputRef.current?.click()} className="p-3 text-slate-500"><PlusCircle className="w-7 h-7" /></button>
            ...
            <textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && (e.preventDefault(), handleSend())} placeholder={t.typeAMessage} className="flex-1 bg-transparent border-none text-lg py-3 focus:outline-none resize-none max-h-40" rows={1} />
            <div className="flex gap-2">
              <button onClick={() => { if(isListening) recognitionRef.current?.stop(); else { recognitionRef.current?.start(); setIsListening(true); }}} className={`p-3 rounded-full transition-all ${isListening ? 'bg-red-500 text-white animate-pulse' : 'text-slate-500'}`}>{isListening ? <StopCircle className="w-7 h-7" /> : <Mic className="w-7 h-7" />}</button>
              <button onClick={() => handleSend()} disabled={!input.trim() && !uploadedAsset} className="p-3.5 rounded-3xl bg-slate-900 text-white"><Send className="w-6 h-6" /></button>
            </div>
          </div>
        </div>
      </div>
